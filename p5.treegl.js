...
var Tree = (function (ext) {
  ...
  */
   ...
   * @type {number}
   */
  ext.pvMatrix = pvMatrix;

  /**
   * @memberof Tree
   * @type {number}
   */
  ext.pvInvMatrix = pvInvMatrix;
...
(function () {
  ...
  p5.RendererGL.prototype._treeLocation = function (vector = Tree.ORIGIN,
    ...
    if (from == Tree.NDC && (to instanceof p5.Matrix || to == Tree.EYE)) {
      return (to == Tree.EYE ? (vMatrix ?? this.vMatrix()) : invMatrix(to)).mult4(this._location({ vector: this._ndcToScreenLocation(vector), pMatrix: pMatrix, vMatrix: vMatrix, pvMatrix: pvMatrix, pvInvMatrix: pvInvMatrix }));
    }
    ...
    if (from == Tree.WORLD && (to instanceof p5.Matrix || to == Tree.EYE)) {
      return (to == Tree.EYE ? (vMatrix ?? this.vMatrix()) : invMatrix(to)).mult4(vector);
    }
    ...
    if (from == Tree.SCREEN && (to instanceof p5.Matrix || to == Tree.EYE)) {
      return (to == Tree.EYE ? (vMatrix ?? this.vMatrix()) : invMatrix(to)).mult4(this._location({ vector: vector, pMatrix: pMatrix, vMatrix: vMatrix, pvMatrix: pvMatrix, pvInvMatrix: pvInvMatrix }));
    }
  ...
  */
  ...
  p5.prototype.vertexShader = function () {
    return this._renderer.vertexShader(...arguments);
  }

  p5.RendererGL.prototype.vertexShader = function ({
    ...
    // vnormal -> normal3 (vVertexNormal currently)
    // vtex -> texCoords2 (vTexCoord currently)
    // position2 -> position2 (already done)
    // position3 -> position3 (already done)
    let vcolor = ~(varyings | ~Tree.vColor) === 0;
    let vtex = ~(varyings | ~Tree.vTexCoord) === 0;
    let vnormal = ~(varyings | ~Tree.vNormal) === 0;
    let position2 = ~(varyings | ~Tree.position2) === 0;
    let position3 = ~(varyings | ~Tree.position3) === 0;
    let pmv = ~(uniforms | ~Tree.pmvMatrix) === 0;
    let p = ~(uniforms | ~Tree.pMatrix) === 0;
    let v = ~(uniforms | ~Tree.vMatrix) === 0;
    let e = ~(uniforms | ~Tree.eMatrix) === 0;
    let mv = ~(uniforms | ~Tree.mvMatrix) === 0;
    let m = ~(uniforms | ~Tree.mMatrix) === 0;
    let n = (~(uniforms | ~Tree.nMatrix) === 0) || vnormal;
    let pv = ~(uniforms | ~Tree.pvMatrix) === 0;
    let pvInv = ~(uniforms | ~Tree.pvInvMatrix) === 0;
    // /*
    const target = `gl_Position = ${pmv ? 'pmvMatrix *' :
      `${p && mv ? 'pMatrix * mvMatrix *' :
        `${p && v && m ? 'pMatrix * vMatrix * mMatrix *' :
          ''}`}`} vec4(aPosition, 1.0)`;
    // */
    /*
    const target = `gl_Position = ${pmv ? 'pmvMatrix *' :
      `${p && mv ? 'pMatrix * mvMatrix *' :
        `${p && v && m ? 'pMatrix * vMatrix * mMatrix *' :
          ''}`}`} vec4(aPosition, 1.0)`;
    */
    let result = `//  generated by treegl
    precision highp float;
    attribute vec3 aPosition;
    attribute vec2 aTexCoord;
    attribute vec4 aVertexColor;
    ${vnormal ? 'attribute vec3 aNormal;' : ''}
    ${pmv ? 'uniform mat4 pmvMatrix;' : ''}
    ${p ? 'uniform mat4 pMatrix;' : ''}
    ${v ? 'uniform mat4 vMatrix;' : ''}
    ${e ? 'uniform mat4 eMatrix;' : ''}
    ${mv ? 'uniform mat4 mvMatrix;' : ''}
    ${m ? 'uniform mat4 mMatrix;' : ''}
    ${n ? 'uniform mat3 nMatrix;' : ''}
    ${pv ? 'uniform mat4 pvMatrix;' : ''}
    ${pvInv ? 'uniform mat4 pvInvMatrix;' : ''}
    ${vcolor ? 'varying vec4 vVertexColor;' : ''}
    ${vtex ? 'varying vec2 vTexCoord;' : ''}
    ${vnormal ? 'varying vec3 vVertexNormal;' : ''}
    ${position2 ? 'varying vec2 position2;' : ''}
    ${position3 ? 'varying vec3 position3;' : ''}
    void main() {
      ${vcolor ? 'vVertexColor = aVertexColor;' : ''}
      ${vtex ? 'vTexCoord = aTexCoord;' : ''}
      ${vnormal ? 'vVertexNormal = normalize(nMatrix * aNormal);' : ''}
