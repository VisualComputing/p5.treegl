...
Post-effects are an essential part of dynamic visual rendering, allowing for the application of various shader effects to a rendered scene. These effects are managed through an `effects()` array, offering flexibility to dynamically alter the rendering order or modify the effects stack as needed. For example, effects can easily be reordered with a simple swap operation: `[effects()[0], effects()[1]] = [effects()[1], effects()[0]]`.

1. `addEffect(key, shader, [index = -1])`: Adds a new effect to the post-effects array, identified by a unique `key` and associated with a `shader`. Optionally, an `index` can be specified to insert the effect at a specific position in the array; if not provided, the effect is added to the end. The `shader` should be an instance of [p5.Shader](https://p5js.org/reference/#/p5.Shader). The effect will be applied in the order it appears in the array when `applyEffects` is called.
2. `removeEffect(key)`: Removes the effect associated with the specified `key` from the post-effects array. Returns an object containing the `key`, `shader`, and `index` of the removed effect, or `undefined` if no effect with the specified key is found.
3. `resetEffects()`: Clears all the effects from the post-effects array, effectively resetting it to an empty state. This is useful for starting fresh with a new set of effects or clearing out effects that are no longer needed.
4. `effects()`: Returns the current post-effects array. Each effect is an object containing the `key`, `shader`, and a `target` which is a [p5.Framebuffer](https://p5js.org/reference/#/p5.Framebuffer) instance used to apply the shader.
5. `effect(key)`: Retrieves the effect associated with the specified `key`. If the effect exists, it returns an object containing the `key`, `shader`, and `target`. If no effect with the specified key is found, it returns `undefined`.
6. `applyEffects(layer, uniforms, flip)`: Sequentially applies all effects (in the order they were added) to the source [p5.Framebuffer](https://p5js.org/reference/#/p5.Framebuffer) `layer`. When `uniforms` is just an object, it is directly used as the uniforms for each shader, assuming a `sampler2D uniform blender` is defined in all the shader effects as shared layer. Alternatively, uniforms can be a mapping where each key corresponds to an effect's identifier and its value is a function that dynamically generates uniform values based on a `shared_layer` param state. The `flip` boolean indicates whether the final image should be vertically flipped. This method processes each effect, applying its shader with the corresponding uniforms (via `applyShader`), and returns the final processed layer, now modified by all effects.
