...
Post-effects are an essential part of dynamic visual rendering, allowing for the application of various shader effects to a rendered scene. These effects are managed through an `effects()` queue, offering flexibility to dynamically alter the rendering order or modify the effects queue as needed. For example, effects can easily be reordered with a simple swap operation: `[effects()[0], effects()[1]] = [effects()[1], effects()[0]]`. Effects may be added with `addEffect(key, shader)`, removed with `removeEffect(key)`, retrieved with `effect(key)`, and sequentially be applied to a source `layer` with `applyEffects(layer, uniforms)`.
...
4. `effects()`: Returns the current post-effects queue (implemented as a js array). Each effect is an object containing the `key`, `shader`, and a `target` which is a [p5.Framebuffer](https://p5js.org/reference/#/p5.Framebuffer) instance used to apply the shader.
