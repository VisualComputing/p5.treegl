...
4. `parseShader(fragSrc, [vertSrc], [matrices = Tree.NONE], [uniformsUIConfig], [key], [successCallback], [failureCallback])`: A high-level dispatcher function that determines whether to call `readShader` or `makeShader` based on the input parameters (where `fragSrc` may be either `fragFilename` or `fragStr` and `vertSrc` may be either `vertFilename` or `vertStr`).
