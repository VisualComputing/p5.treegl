...
1. `parseGeometry(fn, ...args)`: Captures geometry by running `fn` with `args`, then returns a [p5.Geometry](https://p5js.org/reference/#/p5.Geometry) object. Applies `colors` from `args` if specified, or clears them, and [computes normals](https://archive.p5js.org/reference/#/p5.Geometry/computeNormals).
